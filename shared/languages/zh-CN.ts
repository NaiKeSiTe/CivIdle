export const ZH_CN = {
   ...
   AtlasMountainsDesc: "周围 2 地块范围内的所有建筑获得 +N 生产乘数。 N 为该奇观范围内建筑生产但未被消耗的不相同资源数量",
   AtomicBomb: "原子弹",
   AtomicFacility: "原子设施",
   AtomicTheory: "原子理论",
   Atomium: "原子塔",
   AtomiumDescV2: "周围 2 地块范围内所有产出科学的建筑，获得 +5 生产乘数。其产生的科学，等同于周围 2 地块范围内的科学总产出量（包括产出科学的奇观，例如：牛津大学）。建造完成时，一次性生成一批科学，生成量等同于已解锁最昂贵科技所需",
   AuroraBorealis: "北极光",
   ...
   BritishMuseumDesc: "建造完成后，可以变更为其他文明的奇观（蓬皮杜文艺中心，巨港以及自然奇观除外。仅有一次变更机会，请慎重考虑）",
   ...
   CathedralOfBrasiliaDesc:
      "如果周围 2 地块范围的每个建筑形成一个完整的生产链，即范围内一个建筑生产的资源被范围内另一个建筑消耗（范围当中相同产出不同建筑也会被归入完整的生产链中），那么参入该完整的生产链中的每种建筑为其所有相同建筑建筑提供 +X 生产乘数。X 为生产链中不相同建筑数量（最大值为18。奇观、旅馆、仓库、市集、克隆工厂和克隆实验室除外）",
   ...
   EastIndiaCompanyDescV2:
      "当您通过贸易获得的所有物品都会获得相应贸易点数（包括税收，物品的每 1 价值 = 1 贸易点数）。该奇观累积您获得的贸易点数。每周期自动消耗 2000 贸易点数, 使所有建筑每相邻一个运转的商队旅馆获得 +0.5 生产乘数。该奇观可消耗贸易点数进行升级，并且每一次额外升级获得 +0.5 生产乘数",
   Education: "教育学",
   ...
   Gameplay: "游戏设置",
   GangesRiver: "恒河",
   GangesRiverDescV3: "周围 1 地块范围内的每种建筑（产出劳动者建筑除外），每种建筑所有相同建筑建筑获得 +N 生产乘数，N 为解锁该建筑时代的时代智慧等级",
   Garment: "服装",
   ...
   GreatSphinx: "狮身人面像",
   GreatSphinxDescV2: "周围 2 地块范围，所有 II 阶及以上建筑，获得 +N 生产乘数（ N 为其相邻相相同建筑建筑数量）",
   GreatWall: "长城",
   ...
   Herding: "放牧",
   Hermitage: "冬宫博物馆",
   HermitageDesc: "该奇观可被升级，每一级为周围 2 地块范围内各种类型建筑及其所有相同建筑建筑提供 +1 存储乘数",
   Herodotus: "希罗多德",
   HideSpinner: "隐藏运转动画",
   ...
   MiddleClickDescHTML: "选取你要<b>复制蓝图的建筑</b>（包括建造中的）点击，别点击任何地块！直接<b>按鼠标中键</b>指着想要建设相同建筑建筑的地块",
   ...
   OpetV2: "欧佩特：狮身人面像额外为周围 2 地块范围，所有 II 阶及以上建筑，提供 +N 生产等级（ N 为其相邻相相同建筑建筑数量）",
   OpticalFiber: "光纤",
   ...
   ResourceExportToSameTypeTooltip: "允许其它相同建筑建筑从此建筑运送资源",
   ...
   Sputnik1: "伴侣号",
   Sputnik1DescV3:
      "该奇观可被升级，每一级为所有冷战时代的伟人（包括未拥有过的）提供 +1 等级的本轮效果（不包括奇观伟人）。每一级为航天器发射场提供 +1 生产和存储乘数。为航天器发射场提供 +N 生产和存储乘数，N 为冷战时代智慧等级。",
   Stable: "马厩",
   Stadium: "体育场",
   StartFestival: "欢度佳节吧！",
   Stateship: "国家状态",
   ...
   StatueOfLibertyDesc: "与该奇观相邻建筑，获得 +N 生产、劳动者能力以及存储乘数。 N 为被加成建筑其相邻相同建筑建筑数量（例：相邻自由女神像的A建筑，A建筑相邻 3 个相同建筑，则A建筑获得 +3 生产、劳动者能力以及存储乘数）",
   StatueOfZeus: "宙斯神像",
   ...
   TowerOfBabel: "巴别塔",
   TowerOfBabelDesc: "可为与该奇观相邻的建筑所有相同建筑建筑提供 +2 生产乘数（多次相邻不可叠加），需满足相同建筑建筑中至少有一座运转中建筑与奇观相邻",
   ...
   TutorialBuildWheatFarmHTML: "要快速建造大量的建筑，你可以<b>选择一个建筑地块</b>，别点击任何地块！直接<b>按鼠标中键</b>指着想要建设相同建筑建筑的地块——这将自动复制选定的建筑到空白地块上。",
   ...
   YearOfTheSnakeDesc: "建造完成后，进入一个新时代时，由每一个已解锁时代获取一位伟人改为获取相同数量的当前时代伟人。周围 2 地块范围内的所有建筑，获得 +1 生产乘数。该奇观可被升级，并且每一次额外升级为周围 2 地块范围内的所有建筑提供 +1 生产乘数。该奇观只能在农历新年期间（2.10 ~ 2.24）建造",
